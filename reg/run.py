...
DEFAULT_REGISTER_CONFIG = RUNTIME_REGISTER_CONFIG_FILE
...
def load_register_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise ValueError(f"注册配置不是对象: {path}")
    return raw
    ...
    *,
    ...
    try:
        ...
        if not isinstance(data, dict):
            raise RuntimeError("导入返回不是 JSON 对象")
        return data
    finally:
        session.close()
...
def fetch_cloud_account_summary(
    *,
    ...
    try:
        ...
        data = response.json()
        if not isinstance(data, dict):
            raise RuntimeError("云端账号统计返回不是 JSON 对象")
        return data
    finally:
        session.close()
...
def run_local_register_job(
    *,
    ...
    if not import_only:
        ...
        accounts_path, failures_path = save_results(accounts, failures, output_dir)
        logger(f"成功账号已保存: {accounts_path}")
        logger(f"失败记录已保存: {failures_path}")
    elif not accounts:
    ...
    return {
        "accounts": accounts,
        "failures": failures,
        "accounts_path": str(accounts_path) if accounts_path else "",
...
def check_cloud_and_refill(
    *,
    ...
    summary_payload = fetch_cloud_account_summary(server=server, auth_key=auth_key)
    ...
    logger(f"云端当前有效账号数: {active}，目标下限: {min_active_accounts}")
    if active >= max(0, int(min_active_accounts)):
        logger("云端有效账号数量充足，跳过本地注册")
        return {"skipped": True, "summary": summary_payload}
    logger("云端有效账号数量不足，开始本地注册补号")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default=str(DEFAULT_REGISTER_CONFIG), help="注册配置文件路径，默认 data/register.json")
    ...
    parser.add_argument("--enable-flaresolverr", action="store_true", help="本地注册启用 FlareSolverr；默认关闭")
    parser.add_argument("--flaresolverr-url", default="", help="FlareSolverr 地址；留空时回退到 register.json 配置")
    parser.add_argument("--accounts-file", default="", help="已有账号文件路径；提供后可直接导入云端")
    ...
    parser.add_argument("--auth-key", default="", help="云端管理员 auth-key，用于导入账号")
    parser.add_argument("--min-active-accounts", type=int, default=None, help="云端有效账号低于这个值时才执行本地注册并导入")
