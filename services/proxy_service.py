...
def get_urllib_proxy_error(proxy: str = "", account: dict | None = None) -> str:
    profile = proxy_settings.get_profile(account=account, proxy=proxy, resource=True, upstream=True)
...
def test_proxy(url: str = "", *, timeout: float = 15.0) -> dict:
    ...
    started = time.perf_counter()
    chatgpt_session = Session(impersonate="edge101", verify=True, proxy=candidate)
    try:
        response = chatgpt_session.get(
