...
def is_web_search_chat_request(body: dict[str, Any]) -> bool:
    ...
    return (
        ...
        or any(model == prefix or model.startswith(f"{prefix}-") for prefix in SEARCH_CHAT_MODEL_PREFIXES)
    )
...
def _readable_annotation_part(parts: list[str]) -> str:
    for part in parts:
        value = part.strip()
        lower = value.lower()
        if value and not (lower.startswith(("turn", "source", "sources")) or re.fullmatch(r"\d+", value)):
            return value
    return ""
