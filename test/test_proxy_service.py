...
class ProxyServiceTests(unittest.TestCase):
    ...
    def test_runtime_proxy_is_limited_to_upstream_scope_by_default(self) -> None:
        ...
        self.assertEqual(store.build_session_kwargs()["proxy"], "http://legacy.example:8080")
    ...
    def test_proxy_test_error_redacts_proxy_credentials(self) -> None:
        ...
        with (
            patch("services.proxy_service.Session", FailingSession),
            patch(
                "services.proxy_service._test_chatgpt_with_urllib",
                return_value={"ok": True, "status": 200, "latency_ms": 1, "url": "", "error": None},
            ),
            patch("services.proxy_service._lookup_exit_ip", return_value={}),
        ):
