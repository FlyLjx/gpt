...
def export_to_json(output_file: str):
    ...
    output_path.write_text(
        json.dumps({"accounts": accounts, "auth_keys": auth_keys}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
...
def import_from_json(input_file: str):
    ...
    try:
        ...
        if isinstance(payload, list):
            ...
            if not isinstance(accounts, list) or not isinstance(auth_keys, list):
                print(f"[migrate] Error: Invalid JSON format, expected accounts/auth_keys arrays")
                sys.exit(1)
        else:
            print(f"[migrate] Error: Invalid JSON format, expected object or array")
            sys.exit(1)
