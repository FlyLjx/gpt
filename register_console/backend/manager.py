...
class RegWebManager:
    ...
    def list_output_files(self) -> list[dict[str, Any]]:
        output_dir = self._resolve_output_dir(self._load_settings())
    ...
    def cloud_summary(self) -> dict[str, Any]:
        ...
        payload = fetch_cloud_account_summary(server=server, auth_key=auth_key)
        self._append_log(
            f"云端有效账号数: {int(payload.get('valid_account_count') or 0)}",
            level="debug",
        )
        return payload
    ...
    def _finish_busy(self) -> None:
        with self._state_lock:
            ...
            self._current_task = ""
            if self._monitoring:
                self._current_hint = "正在循环检查云端账号数量，低于阈值时会自动补号"
            else:
                self._current_hint = "可以开始本地注册、导入云端或开启自动补号监控"
    ...
    def _run_threaded(
        ...
        *,
        task_name: str,
        target,
        allow_when_monitoring: bool = False,
    ) -> None:
    ...
    def start_monitor(self) -> None:
        ...
        self._append_log(f"已开启循环监控，间隔 {interval_seconds}s，低于阈值时自动补号")
