...
from reg.runtime import DATA_DIR, OUTPUT_DIR, ensure_runtime_environment
from register_console.backend.manager import RegWebManager
...
def create_app(manager: RegWebManager | None = None) -> FastAPI:
    ...
    async def files() -> dict:
        return {"items": runtime_manager.list_output_files()}
    ...
    async def start_import(file: UploadFile = File(...)) -> dict:
        suffix = Path(file.filename or "accounts.json").suffix or ".json"
        target = OUTPUT_DIR / "imports" / f"upload-{time.time_ns()}{suffix}"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(await file.read())
        try:
            runtime_manager.start_import(target)
